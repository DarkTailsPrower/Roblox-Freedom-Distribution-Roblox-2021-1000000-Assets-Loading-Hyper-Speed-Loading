...
do
	...
	function Util.SetGUIInsetBounds(x, y)
		local success, _ = pcall(function()
			GuiService:SetGlobalGuiInset(0, x, 0, y)
		end)
		if not success then
			pcall(function()
				GuiService:SetGlobalSizeOffsetPixel(-x, -y)
			end) -- Legacy GUI-offset function
		end
	end
...
local function CreateSystemChatMessage(settings, chattedMessage)
		...
		if gui then
			if instant then
				...
				for _, object in pairs(tweenableObjects) do
					table.insert(
						this.FadeRoutines,
						Util.PropertyTweener(object, "TextTransparency", object.TextTransparency, 1, 1, Util.Linear)
					)
					table.insert(
						this.FadeRoutines,
						Util.PropertyTweener(
							object,
	...
	local function CreateMessageGuiElement()
		...
		local systemMessageSize =
			Util.GetStringTextBounds(systemMessageDisplayText, this.Settings.Font, fontSize, UDim2.new(0, 400, 0, 1000))
...
InputService.InputBegan:connect(function(inputObject, isProcessed)
	...
	if
		(inputType == Enum.UserInputType.Touch and inputObject.UserInputState == Enum.UserInputState.Begin)
...
local function CreatePlayerChatMessage(settings, playerChatType, sendingPlayer, chattedMessage, receivingPlayer)
	...
	function this:FormatPlayerNameText()
		...
		if
			this.PlayerChatType == Enum.PlayerChatType.Whisper
			and this.SendingPlayer
			and this.SendingPlayer == Player
		then
				...
				if this.UserNameDot then
					table.insert(
						...
						Util.PropertyTweener(
							this.UserNameDot,
							"ImageTransparency",
							this.UserNameDot.ImageTransparency,
							1,
							1,
							Util.Linear
						)
	...
	local function CreateMessageGuiElement()
		...
		local chatTypeSize = chatTypeDisplayText
				and Util.GetStringTextBounds(chatTypeDisplayText, this.Settings.Font, fontSize)
			or Vector2.new(0, 0)
		...
		local chatMessageSize = Util.GetStringTextBounds(
			chatMessageDisplayText,
			this.Settings.Font,
			fontSize,
			UDim2.new(0, 400 - 5 - playerNameSize.X, 0, 1000)
		)
		...
		if
			this.SendingPlayer
			and this.SendingPlayer == Player
			and this.PlayerChatType == Enum.PlayerChatType.Whisper
		then
...
local function CreateChatBarWidget(settings)
	...
	this.ChatMatchingRegex = {
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/w ") .. "(%w+_?%w+)")
		end] = "Whisper",
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/whisper ") .. "(%w+_?%w+)")
		end] = "Whisper",

		[function(chatBarText)
			return string.find(chatBarText, "^%%")
		end] = "Team",
		[function(chatBarText)
			return string.find(chatBarText, "^%(TEAM%)")
		end] = "Team",
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/t"))
		end] = "Team",
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/team"))
		end] = "Team",

		[function(chatBarText)
			return string.find(chatBarText, nocase("^/a"))
		end] = "All",
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/all"))
		end] = "All",
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/s"))
		end] = "All",
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/say"))
		end] = "All",

		[function(chatBarText)
			return string.find(chatBarText, nocase("^/e"))
		end] = "Emote",
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/emote"))
		end] = "Emote",
		...
		[function(chatBarText)
			return string.find(chatBarText, nocase("^/help"))
		end] = "Help",

		[function(chatBarText)
			return string.find(chatBarText, nocase("^/block ") .. "(%w+_?%w+)")
		end] = "Block",

		[function(chatBarText)
			return string.find(chatBarText, nocase("^/unblock ") .. "(%w+_?%w+)")
		end] = "Unblock",

		[function(chatBarText)
			return string.find(chatBarText, nocase("^/mute ") .. "(%w+_?%w+)")
		end] = "Mute",

		[function(chatBarText)
			return string.find(chatBarText, nocase("^/unmute ") .. "(%w+_?%w+)")
		end] = "Unmute",
	}
	...
	local function HookUpEvents()
		...
		if this.ChatBarLostFocusEvent then
			this.ChatBarLostFocusConn = this.ChatBarLostFocusEvent:connect(function(...)
				this:OnChatBarFocusLost(...)
			end)
		end
		...
		if this.ChatBar then
			...
			for regexFunc, actionType in pairs(this.ChatMatchingRegex) do
				...
				if start and finish then
					...
					local whitespaceAfterSlashCommand =
						string.find(string.sub(chatBarText, finish + 1, finish + 1), "%s")
					if
						(not requireWhitespaceAfterChatMode and finish == #chatBarText) or whitespaceAfterSlashCommand
					then
	...
	function this:OnChatBarBoundsChanged()
		if this.ChatBarContainer and this.ChatBar then
			...
			if this.ChatBar.Visible and not this.ChatBar.TextFits then
				local textBounds = Util.GetStringTextBounds(
					this.ChatBar.Text,
					this.ChatBar.Font,
					this.ChatBar.FontSize,
					UDim2.new(0, this.ChatBar.AbsoluteSize.X, 0, 1000)
				)
	...
	function this:SetMessageMode(newMessageMode)
		...
		if
			...
			or (newMessageMode == "Whisper" and this.ChatModeText and chatRecipientText ~= this.ChatModeText.Text)
		then
			if this.ChatModeText then
				this.MessageMode = newMessageMode
				if newMessageMode == "Whisper" then
					local chatRecipientTextBounds =
						Util.GetStringTextBounds(chatRecipientText, this.ChatModeText.Font, this.ChatModeText.FontSize)
				...
				if cText ~= "" then
					if self:FloodCheck() then -- and not didMatchSlashCommand then
						...
						elseif currentMessageMode == "Whisper" then
							if self.TargetWhisperPlayer then
								if self.TargetWhisperPlayer == Player then
									...
									pcall(function()
										PlayersService:WhisperChat(cText, self.TargetWhisperPlayer)
									end)
								end
							else
						...
						elseif currentMessageMode == "All" then
							...
							spawn(function()
								error("ChatScript: Unknown Message Mode of " .. tostring(currentMessageMode))
							end)
						end
...
local function CreateChatWindowWidget(settings)
	...
	local function PointInChatWindow(pt)
		...
		return (point0.X <= pt.X and point1.X >= pt.X and point0.Y <= pt.Y and point1.Y >= pt.Y)
	end
	...
	function this:SetCanvasPosition(newCanvasPosition)
		if this.ScrollingFrame then
			local maxSize = Vector2.new(
				math.max(0, this.ScrollingFrame.CanvasSize.X.Offset - this.ScrollingFrame.AbsoluteWindowSize.X),
				math.max(0, this.ScrollingFrame.CanvasSize.Y.Offset - this.ScrollingFrame.AbsoluteWindowSize.Y)
			)
			this.ScrollingFrame.CanvasPosition = Vector2.new(
				Util.Clamp(0, maxSize.X, newCanvasPosition.X),
	...
	function this:ScrollToBottom()
		if this.ScrollingFrame then
			this:SetCanvasPosition(
				Vector2.new(this.ScrollingFrame.CanvasPosition.X, this.ScrollingFrame.CanvasSize.Y.Offset)
			)
		end
	end
	...
	function this:FadeIn(duration, lockFade)
		if not FadeLock then
			...
			this.BackgroundTweener = Util.PropertyTweener(
				this.ChatContainer,
			...
			this.ChatWindowPagingConn = InputService.InputBegan:connect(function(inputObject)
				...
				if key == Enum.KeyCode.PageUp then
					this:SetCanvasPosition(
						this.ScrollingFrame.CanvasPosition - Vector2.new(0, this.ScrollingFrame.AbsoluteWindowSize.Y)
					)
	...
	function this:FadeInChats()
		if this.ChatsVisible == true then
			return
		end
	...
	function this:FadeOutChats()
		...
		for index, message in pairs(this.Chats) do
			...
			if messageGui and this.ScrollingFrame then
				...
				if
					messageGui.AbsolutePosition.Y
						> (this.ScrollingFrame.AbsolutePosition + this.ScrollingFrame.AbsoluteWindowSize).Y
					or messageGui.AbsolutePosition.Y + messageGui.AbsoluteSize.Y
						< this.ScrollingFrame.AbsolutePosition.Y
				then
					instant = true
				end
			end
	...
	function this:OnResize()
		...
		wait()
		if this.ScrollingFrame then
			if currentResizeCount ~= ResizeCount then
				return
			end
			local scrollingFrameAbsoluteSize = this.ScrollingFrame.AbsoluteWindowSize
			if
				scrollingFrameAbsoluteSize ~= nil
				and scrollingFrameAbsoluteSize.X > 0
				and scrollingFrameAbsoluteSize.Y > 0
			then
				local ySize = 0
				...
				if this.MessageContainer and this.ScrollingFrame then
					this.MessageContainer.Size =
						UDim2.new(this.MessageContainer.Size.X.Scale, this.MessageContainer.Size.X.Offset, 0, ySize)
					this.MessageContainer.Position = UDim2.new(0, 0, 1, -this.MessageContainer.Size.Y.Offset)
					this.ScrollingFrame.CanvasSize = UDim2.new(
						this.ScrollingFrame.CanvasSize.X.Scale,
						this.ScrollingFrame.CanvasSize.X.Offset,
						this.ScrollingFrame.CanvasSize.Y.Scale,
						ySize
					)
				end
			end
	...
	function this:AddDeveloperSystemChatMessage(informationTable)
		...
		if informationTable["Text"] and type(informationTable["Text"]) == "string" then
			...
			if
				typeof(informationTable.FontSize) == "EnumItem"
				and informationTable.FontSize.EnumType == Enum.FontSize
			then
	...
	function this:AddChatMessage(playerChatType, sendingPlayer, chattedMessage, receivingPlayer, silently)
		...
		if this:FilterMessage(playerChatType, sendingPlayer, fixedChattedMessage, receivingPlayer) then
			local chatMessage = CreatePlayerChatMessage(
				this.Settings,
				playerChatType,
				sendingPlayer,
				fixedChattedMessage,
	...
	function this:IsScrolledDown()
		if this.ScrollingFrame then
			...
			return yCanvasSize < yContainerSize or yCanvasSize - yScrolledPosition <= yContainerSize + 5 -- a little wiggle room
		end
		return false
	end
	...
	local function CreateChatWindow()
		...
		if Util:IsTouchDevice() then
			...
			this.MouseEnterFrameConn = this.ChatContainer.MouseEnter:connect(function()
				lastEnterTime = tick()
				if
					this.BackgroundTweener
					and not this.BackgroundTweener:IsFinished()
					and not this.BackgroundVisible
				then
					this:FadeIn()
				end
			end)
			...
			this.MouseMoveConn = InputService.InputChanged:connect(function(inputObject)
				if inputObject.UserInputType == Enum.UserInputType.MouseMovement then
					...
					if
						this.BackgroundTweener
						and this.BackgroundTweener:GetPercentComplete() < 0.5
						and this.BackgroundVisible
					then
						if not dontFadeOutOnMouseLeave then
							this:FadeOut()
						end
...
local function CreateChat()
	...
	function this:OnPlayerAdded(newPlayer)
		if newPlayer then
			assert(coroutine.resume(coroutine.create(function()
				PlayerPermissionsModule.IsPlayerAdminAsync(newPlayer)
			end)))
		end
		if NON_CORESCRIPT_MODE then
	...
	function this:PrintHelp()
		if this.ChatWindowWidget then
			...
			this.ChatWindowWidget:AddSystemChatMessage(
				"/unblock [PlayerName] - Restore communications with Target Player"
			)
			this.ChatWindowWidget:AddSystemChatMessage(
				"/mute [PlayerName] - Mute in-game communications from Target Player"
			)
			this.ChatWindowWidget:AddSystemChatMessage(
				"/unmute [PlayerName] - Restore in-game communications with Target Player"
			)
		end
	end
	...
	function this:CreateGUI()
		if
			FORCE_CHAT_GUI
			or (Player.ChatMode == Enum.ChatMode.TextAndMenu or RunService:IsStudio())
				and game:GetService("UserInputService"):GetPlatform() ~= Enum.Platform.XBoxOne
		then
			if NON_CORESCRIPT_MODE then
			...
			this.ChatBarWidget.ChatCommandEvent:connect(function(success, actionType, capture)
				...
				elseif actionType == "Block" then
					...
					if playerToBlock then
						spawn(function()
							this:BlockPlayerAsync(playerToBlock)
						end)
					else
						this.ChatWindowWidget:AddSystemChatMessage(
							"Cannot block " .. blockPlayerName .. " because they are not in the game."
						)
					end
				elseif actionType == "Unblock" then
					...
					if playerToBlock then
						spawn(function()
							this:UnblockPlayerAsync(playerToBlock)
						end)
					else
						this.ChatWindowWidget:AddSystemChatMessage(
							"Cannot unblock " .. unblockPlayerName .. " because they are not in the game."
						)
					end
				elseif actionType == "Mute" then
					...
					if playerToMute then
						...
						this.ChatWindowWidget:AddSystemChatMessage(
							"Cannot mute " .. mutePlayerName .. " because they are not in the game."
						)
					end
				elseif actionType == "Unmute" then
					...
					if playerToUnmute then
						...
						this.ChatWindowWidget:AddSystemChatMessage(
							"Cannot unmute " .. unmutePlayerName .. " because they are not in the game."
						)
					end
	...
	local function SetVisbility(newVisibility)
		...
		if InputService.VREnabled then
			...
			if this.Visible then
				...
				panel.localCF = topbarPanel.localCF
	...
	function this:IsFocused(useWasFocused)
		if not self.ChatBarWidget then
			return false
		end
	...
	function this:Initialize()
		...
		if not NON_CORESCRIPT_MODE then
			...
			StarterGui:RegisterSetCore("ChatWindowPosition", function(value)
				if this.ChatWindowWidget and this.ChatBarWidget then
					...
					if value ~= nil and not isBubbleChatOn() then
						...
						this.ChatBarWidget.ChatBarContainer.Position = value
							+ UDim2.new(
								0,
								0,
								this.ChatWindowWidget.ChatContainer.Size.Y.Scale,
								this.ChatWindowWidget.ChatContainer.Size.Y.Offset + 2
							)
					end
				end
			end)
			...
			StarterGui:RegisterSetCore("ChatWindowSize", function(value)
				if this.ChatWindowWidget and this.ChatBarWidget then
					...
					if value ~= nil and not isBubbleChatOn() then
						...
						this.ChatBarWidget.ChatBarContainer.Size = UDim2.new(
							this.ChatWindowWidget.ChatContainer.Size.X.Scale,
							this.ChatWindowWidget.ChatContainer.Size.X.Offset,
							this.ChatBarWidget.ChatBarContainer.Size.Y.Scale,
							this.ChatBarWidget.ChatBarContainer.Size.Y.Offset
						)
						this.ChatBarWidget.ChatBarContainer.Position = this.ChatWindowWidget.ChatContainer.Position
							+ UDim2.new(
								0,
								0,
								this.ChatWindowWidget.ChatContainer.Size.Y.Scale,
			...
			StarterGui:RegisterGetCore("ChatBarDisabled", function()
				return chatBarDisabled
			end)
		end
		...
		pcall(function()
			this.CoreGuiChangedConn = StarterGui.CoreGuiChangedSignal:connect(function(coreGuiType, enabled)
