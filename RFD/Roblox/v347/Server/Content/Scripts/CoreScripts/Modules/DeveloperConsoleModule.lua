...
function DeveloperConsole.new(screenGui, permissions, messagesAndStats)
	...
	do -- Options animation
		...
		local animationToggle = devConsole:GenerateOptionButtonAnimationToggle(
			interiorFrame,
			optionsButton,
			gear,
			tabContainer,
	...
	do -- Options contents/filters
		...
		function messageFilter(message)
			return filterLookup[message.Type]
				and (not containsString or string_find(message.Message:lower(), containsString))
		end
		...
		local function createConsoleTab(
			...
			local tab = devConsole:AddTab(text, tabBody, function(open)
				...
				if open then
					...
					if commandLineVisible then
						if open and not commandLine then
							...
							commandLine.Frame.Size =
								UDim2_new(1, -(Style.HandleHeight + Style.BorderSize * 2), 0, Style.CommandLineHeight)
					...
					window.Size = commandLineVisible and UDim2_new(1, 0, 1, -Style.HandleHeight)
	...
	do -- Stats tabs
		...
		-- Wrapper for :AddTab
		local function createStatsTab(name, text, config, openCallback, filterStats, shownOptionTypes)
		...
		if permissions.MayViewDataStoreBudget then
			...
			local tab, statList =
				createStatsTab("DataStoreBudget", "DataStore Budget", config, openCallback, filterStats, {
					Stats = true,
					Search = true,
				})
...
do -- Chart
	...
	local function CreateGraph(points, statIndex, autoScale)
		...
		local function generateSizeAndPosition(height, position)
			height = height * scale
			return UDim2_new(0, barWidth, height, 0),
		...
		function this.OnPointAdded(this)
			...
			body:TweenPosition(
				UDim2_new(1 - (direction * 0.5 + 0.5), (barWidth + 1) * position * direction, 0, 0),
				"Out",
				"Sine",
				0.25,
				true
	...
	function Methods.CreateChart(devConsole, points, title, statIndex, pointToString)
		...
		local labelCurrent =
			createLabel(frame, "Current", "Current: " .. pointToString(points[#points] and points[#points][statIndex]))
...
function Methods.AddTab(devConsole, text, body, openCallback, visibleCallback)
	...
	function tab.RecordInitialOpen(tab)
		...
		analyticsService:trackEvent(AnalyticsCategory_Game, AnalyticsAction_InitialOpenTab, tab.Body.Name)
	end

	function tab.RecordClickToOpen(tab)
		local analyticsService = game:GetService("AnalyticsService")
		analyticsService:trackEvent(AnalyticsCategory_Game, AnalyticsAction_ClickToOpenOpenTab, tab.Body.Name)
	end
...
do
	...
	function DeveloperConsole.GetPermissions()
		...
		local success, result = pcall(function()
			local url =
				string.format("/users/%d/canmanage/%d", game:GetService("Players").LocalPlayer.UserId, game.PlaceId)
			return game:GetService("HttpRbxApiService")
				:GetAsync(url, Enum.ThrottlingPriority.Default, Enum.HttpRequestType.Default, true)
		end)
		...
		pcall(function()
			permissions.ServerCodeExecutionEnabled = permissions.IsCreator
				and (not settings():GetFFlag("DebugDisableLogServiceExecuteScript"))
		end)
...
do
	...
	function DeveloperConsole.GetMessagesAndStats(permissions)
		...
		local warningsToFilter =
			{ "ClassDescriptor failed to learn", "EventDescriptor failed to learn", "Type failed to learn" }
