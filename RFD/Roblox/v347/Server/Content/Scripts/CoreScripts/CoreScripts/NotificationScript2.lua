...
spawn(function()
	...
	local RemoteEvent_NewFollower = RobloxReplicatedStorage:WaitForChild("NewFollower", 86400)
		or RobloxReplicatedStorage:WaitForChild("NewFollower")
	...
	RemoteEvent_NewFollower.OnClientEvent:connect(function(followerRbxPlayer)
		...
		if FFlagNotificationScript2UseFormatByKey then
			message =
				RobloxTranslator:FormatByKey("NotificationScript2.NewFollower", { RBX_NAME = followerRbxPlayer.Name })
		else
			if FFlagUseNotificationsLocalization then
				message = string.gsub(
					LocalizedGetString("NotificationScript2.NewFollower", message),
					"{RBX_NAME}",
					followerRbxPlayer.Name
				)
			end
		end
	...
	if fromPlayer == LocalPlayer then
		if event == Enum.FriendRequestEvent.Accept then
			...
			if FFlagNotificationScript2UseFormatByKey then
				...
				if FFlagUseNotificationsLocalization then
					detailText = string.gsub(
						LocalizedGetString("NotificationScript2.FriendRequestEvent.Accept", detailText),
						"{RBX_NAME}",
						toPlayer.Name
					)
				end
	...
	if pointsNotificationsActive and userId == LocalPlayer.UserId then
		...
		if pointsAwarded == 1 then
			...
			if FFlagNotificationScript2UseFormatByKey then
				...
				if FFlagUseNotificationsLocalization then
					text = string.gsub(
						LocalizedGetString("NotificationScript2.onPointsAwarded.single", text),
						"{RBX_NUMBER}",
						pointsAwarded
					)
				end
		...
		elseif pointsAwarded > 0 then
			...
			if FFlagNotificationScript2UseFormatByKey then
				...
				if FFlagUseNotificationsLocalization then
					text = string.gsub(
						LocalizedGetString("NotificationScript2.onPointsAwarded.multiple", text),
						"{RBX_NUMBER}",
						pointsAwarded
					)
				end
		...
		elseif pointsAwarded < 0 then
			...
			if FFlagNotificationScript2UseFormatByKey then
				...
				if FFlagUseNotificationsLocalization then
					text = string.gsub(
						LocalizedGetString("NotificationScript2.onPointsAwarded.negative", text),
						"{RBX_NUMBER}",
						math.abs(pointsAwarded)
					)
				end
			end
		else
...
local function createDeveloperNotification(notificationTable)
	if type(notificationTable) == "table" then
		if type(notificationTable.Title) == "string" and type(notificationTable.Text) == "string" then
			...
			local title = autoLocalize and GameTranslator:TranslateGameText(CoreGui, notificationTable.Title)
				or notificationTable.Title
			local text = autoLocalize and GameTranslator:TranslateGameText(CoreGui, notificationTable.Text)
				or notificationTable.Text
...
if not isTenFootInterface then
	...
	gamepadNotifications.Event:connect(function(isSelected)
		...
		if GuiService.SelectedCoreObject then
			ContextActionService:BindCoreAction(
				"LeaveNotificationSelection",
				leaveNotificationFunc,
				false,
	...
	GuiService.Changed:connect(function(prop)
		if prop == "SelectedCoreObject" then
			if
				not GuiService.SelectedCoreObject or not GuiService.SelectedCoreObject:IsDescendantOf(NotificationFrame)
			then
				isPaused = false
			end
		end
